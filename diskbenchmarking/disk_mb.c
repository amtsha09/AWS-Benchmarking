...
void *disk_banchmark_read_megabytes(void)
{
...
/* Allocated the memory to a buffer */
	pcBuffer = (char *)malloc((MEGA_BYTES+1) * sizeof(char ));
...
/* Start time of read proess *********************/
		Start_t = clock();
		for(iStartLoop = 0; iStartLoop < MEGA_BYTES; iStartLoop++)
		{
			c = fgetc(fp);	
...
/* total time for read operation */
		...
		printf("*  Latancy time in msec = %lf\t\t*\n", (Total_t*1000)/((MEGA_BYTES*6)+1));
		printf("*  Speed of Random read in Mb/Sec = %lf\t\t*\n", ((MEGA_BYTES*6)+1)/(Total_t));
...
int main()
{
	...
	while(1)
	{
		...
		{
			case 1:
				iThreadReturn_1 = pthread_create(&Thread[0], NULL, (void *)disk_banchmark_read_megabytes, NULL);
			...
			case 2:
				...
				iThreadReturn_1 = pthread_create(&Thread[0], NULL, (void *)disk_banchmark_read_megabytes, NULL);
				iThreadReturn_2 = pthread_create(&Thread[1], NULL, (void *)disk_banchmark_read_megabytes, NULL);
